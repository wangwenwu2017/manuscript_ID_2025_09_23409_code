...
enhanced_absorption = (enhanced_absorption).reshape(years_num, 12, 721, 1440)
total_absorption_clr = (total_absorption_clr).reshape(years_num, 12, 721, 1440)
total_absorption = (total_absorption).reshape(years_num, 12, 721, 1440)
enhanced_absorption = np.nanmean(total_absorption_clr, axis=1)
...
for i in range(721):  # Latitude loop
    for j in range(1440):  # Longitude loop
        ...
        years = np.arange(2011, 2024)  # 2004-2023 years
...
# ====================== Right: World Map ======================
# Create map
levels = np.linspace(-4, 4, 41)  # 41 levels
...
cbar.set_ticks([-4,-2,0,2,4])  # Set tick positions
# Add overall title
