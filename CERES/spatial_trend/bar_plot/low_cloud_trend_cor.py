...
cloud_low_bins = np.arange(-3, 3.01, 1.2)  # [-2.0, -1.7, -1.4, ..., 0.7, 1.0]
...
bars = plt.bar(low_centers, mean_values, width=0.8,
               color='#be588d', alpha=0.4, edgecolor='#be588d')  # Magenta color for low/mid cloud
...
for j, center in enumerate(low_centers):
    if not np.isnan(mean_values[j]):
        ...
        plt.text(
            ...
            bbox=dict(boxstyle='round, pad=0.2', facecolor='#fcd590', alpha=0.7)  # Light orange background
        )
...
plt.ylim(-1.2, 0)  # Set y-axis limits
