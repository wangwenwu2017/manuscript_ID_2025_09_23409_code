...
for i in range(22):
    for j in range(i + 1, 23):
        ...
        dSW_alb = np.array(((alb_kernel_TOA_clr) - (alb_kernel_TOA_all))) * dalb * 100
        ...
        CRE_Aatm = (toa_all_with - toa_clr_with)
        CRE_Aatm_no = (toa_all_no - toa_clr_no)
        ...
        dSW_q = ((q_kernel_TOA_clr - q_kernel_TOA_all) / dqdt) * np.array(q_after - q_before)  # Q-change contribution
...
np.save('output/dSW_cloud_global_toa', np.array(dSW_cloud_global__))
np.save('output_spatial/dSW_cloud_global_toa', np.array(dSW_cloud))
