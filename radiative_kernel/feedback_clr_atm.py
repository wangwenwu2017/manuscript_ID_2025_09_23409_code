...
for i in range(22):
    for j in range(i + 1, 23):
        ...
        dSW_alb = np.array((alb_kernel_TOA_clr - alb_kernel_SUR_clr)) * dalb * 100
        ...
        dSW_q = np.array((q_kernel_TOA_clr - q_kernel_SUR_clr) / dqdt) * np.array(
...
# Save results
np.save('output/dSW_alb_global_atm_clr', np.array(dSW_alb_global__))
np.save('output/dSW_q_global_atm_clr', np.array(dSW_q_global__))