...
# Extract surface shortwave radiation components
...
# Define saturation specific humidity function outside loop for efficiency
...
for i in range(22):
    for j in range(i + 1, 23):
        ...
        dSW_alb = np.array(
            ((alb_kernel_TOA_all - alb_kernel_SUR_all))) * dalb * 100
        ...
        dSW_q = np.array(
            (((q_kernel_TOA_all - q_kernel_SUR_all)) / dqdt)) * np.array(
...
# Save results
np.save('output/dSW_alb_global_atm_all', np.array(dSW_alb_global__))
np.save('output/dSW_q_global_atm_all', np.array(dSW_q_global__))