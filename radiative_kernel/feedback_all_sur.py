...
for i in range(22):
    for j in range(i + 1, 23):
        # Calculate albedo feedback using clear-sky radiation
        alb_before = us_clr[i * 12:(i + 1) * 12] / ds_clr[i * 12:(i + 1) * 12]
        alb_after = us_clr[j * 12:(j + 1) * 12] / ds_clr[j * 12:(j + 1) * 12]
        dalb = np.array(alb_after) - np.array(alb_before)
        dSW_alb = np.array(alb_kernel_SUR_all) * dalb * 100
        ...
        dSW_q = (q_kernel_SUR_all / dqdt) * np.array(q_after - q_before)  # Q-change contribution
...
# Save results
np.save('output/dSW_alb_global_sur_all', np.array(dSW_alb_global__))
np.save('output/dSW_q_global_sur_all', np.array(dSW_q_global__))