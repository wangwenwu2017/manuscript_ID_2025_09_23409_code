...
for i in range(22):
    for j in range(i + 1, 23):
        ...
        dSW_alb = np.array(alb_kernel_TOA_clr) * dalb * 100
        ...
        dSW_q = (q_kernel_TOA_clr / dqdt) * np.array(q_after - q_before)  # Q-change contribution
...
# Save results
np.save('output/dSW_alb_global_toa_clr', np.array(dSW_alb_global__))
np.save('output/dSW_q_global_toa_clr', np.array(dSW_q_global__))