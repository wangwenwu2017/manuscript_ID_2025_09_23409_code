...
start_time = '2001-01'
end_time = '2024-12'
ds_all = radiative_ds['sfc_sw_down_all_mon'].sel(time=slice(start_time, end_time))
us_all = radiative_ds['sfc_sw_up_all_mon'].sel(time=slice(start_time, end_time))

ds_clr = radiative_ds['sfc_sw_down_clr_t_mon'].sel(time=slice(start_time, end_time))
us_clr = radiative_ds['sfc_sw_up_clr_t_mon'].sel(time=slice(start_time, end_time))

dt_all = radiative_ds['solar_mon'].sel(time=slice(start_time, end_time))
ut_all = radiative_ds['toa_sw_all_mon'].sel(time=slice(start_time, end_time))

dt_clr = radiative_ds['solar_mon'].sel(time=slice(start_time, end_time))
ut_clr = radiative_ds['toa_sw_clr_t_mon'].sel(time=slice(start_time, end_time))
...
for i in range(22):
    for j in range(i + 1, 23):
        ...
        dSW_alb = np.array(((alb_kernel_SUR_clr) - (alb_kernel_SUR_all))) * dalb * 100
        ...
        CRE_Aatm = (sfc_all_with - sfc_clr_with)
        CRE_Aatm_no = (sfc_all_no - sfc_clr_no)
        ...
        dSW_q = ((q_kernel_SUR_clr - q_kernel_SUR_all) / dqdt) * np.array(q_after - q_before)  # Q-change contribution
        dSW_q[np.where(np.isinf(dSW_q))] = np.nan

        cre_before = (ds_all[i * 12:(i + 1) * 12] - us_all[i * 12:(i + 1) * 12]) - (
                ds_clr[i * 12:(i + 1) * 12] - us_clr[i * 12:(i + 1) * 12])
        cre_after = (ds_all[j * 12:(j + 1) * 12] - us_all[j * 12:(j + 1) * 12]) - (
...
np.save('output/dSW_cloud_global_sur', np.array(dSW_cloud_global__))
np.save('output_spatial/dSW_cloud_global_sur', np.array(dSW_cloud))
# np.save('dSW_alb_global', np.array(dSW_alb_global__))
# np.save('dSW_q_global', np.array(dSW_q_global__))