...
# Process SSP585 surface data
d_sur, b = adaptive_combine_arrays(models, '_ssp585_sur.npy')
...
for i in range((np.array(d_sur).shape[0]) + 1):
    if i != 13:
        # Calculate Theil-Sen slope for surface data (2015-2100)
        slope, intercept, low_hist, high_hist = theilslopes(d_sur[i, :], np.arange(2015, 2100, 1))
        ...
        tau, p_value = kendalltau(np.arange(2015, 2100, 1), d_sur[i, :])
        if p_value > 0.05:
            print(f"Model {models[i]} surface trend not significant (p={p_value:.4f})")
    else:
        # Calculate multi-model ensemble (MME) trend
        slope, intercept, low_hist, high_hist = theilslopes(np.mean(d_sur, axis=0), np.arange(2015, 2100, 1))
        trend_sur_hist[i] = slope * 10
        low_hist_sur_arr[i] = low_hist * 10
        high_hist_sur_arr[i] = high_hist * 10

        tau, p_value = kendalltau(np.arange(2015, 2100, 1), np.mean(d_sur, axis=0))
        if p_value > 0.05:
            print(f"Model {models[i]} surface trend not significant (p={p_value:.4f})")

# Process SSP585 atmospheric data
d, b = adaptive_combine_arrays(models, '_ssp585.npy')
...
for i in range((np.array(d).shape[0]) + 1):
    if i != 13:
        slope, intercept, low_hist, high_hist = theilslopes(d[i, :], np.arange(2015, 2100, 1))
        trend_hist[i] = slope * 10
        low_hist_arr[i] = low_hist * 10
        high_hist_arr[i] = high_hist * 10

        tau, p_value = kendalltau(np.arange(2015, 2100, 1), d[i, :])
        if p_value > 0.05:
            print(f"Model {models[i]} atmospheric trend not significant (p={p_value:.4f})")
    else:
        # Calculate MME trend for atmospheric data
        slope, intercept, low_hist, high_hist = theilslopes(np.mean(d, axis=0), np.arange(2015, 2100, 1))
        trend_hist[i] = slope * 10
        low_hist_arr[i] = low_hist * 10
        high_hist_arr[i] = high_hist * 10

        tau, p_value = kendalltau(np.arange(2015, 2100, 1), np.mean(d, axis=0))
        if p_value > 0.05:
            print(f"Model {models[i]} atmospheric trend not significant (p={p_value:.4f})")
...
plt.ylim(-0.3, 0.9)  # Set y-axis limits
